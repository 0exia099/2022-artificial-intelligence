...
def ctof():
    tf = float(e2.get())		# e1에서 문자열을 읽어서 부동소수점형으로 변경
    tf = (tc-32.0)* 9/5+32	# 화씨 온도를 섭씨 온도로 변환한다. 
    e2.delete(0, END)			# 처음부터 끝까지 지운다.
    e2.insert(0, str(tf))    
...
Button(window, text="섭씨->화씨", command=process).grid(row=2, column=0)
Button(window, text="화씨->섭씨", command=process).grid(row=2, column=1)
Button(window, text="Reset", command=process).grid(row=2, column=2, columnspan=2, sticky=W+E+N+S)
window.mainloop()